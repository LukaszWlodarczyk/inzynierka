...
# Load Dataframe
data = load_standard_data_frame(get_hourly_last_3_months_data('LTC'))
...
# Build LSTM model
model = Sequential()
model.add(LSTM(units=8, return_sequences=False, input_shape = (X_TRAIN.shape[1], 6)))
...
model.save(f'../saved_model/LTC_model_for_hours_from_full_data__val_lose_{val_loss[-1]}')
dump(sc, f'../saved_model/LTC_scaler_for_model_for_full_data_hours_val_lose_{val_loss[-1]}')

save_info(ONE_BATCH_SIZE,BATCH_SIZE,EPOCHS,TRAIN_TEST_SPLIT_POINT,loss,val_loss,model,'LTC','full_data_hours')
