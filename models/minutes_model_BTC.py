...
# Load Dataframe
data = pd.DataFrame(get_minute_last_day_data('BTC'), columns=['close'])
...
model.save(f'../saved_model/BTC_model_for_minutes_val_lose_{val_loss[-1]}')
dump(sc, f'../saved_model/BTC_scaler_for_model_for_minutes_val_lose_{val_loss[-1]}')

save_info(ONE_BATCH_SIZE,BATCH_SIZE,EPOCHS,TRAIN_TEST_SPLIT_POINT,loss,val_loss,model,'BTC','minutes')
