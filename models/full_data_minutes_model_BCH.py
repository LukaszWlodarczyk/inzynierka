...
# Load Dataframe
data = load_standard_data_frame(get_minute_last_day_data('BCH'))
...
# Build LSTM model
model = Sequential()
model.add(LSTM(units=6, return_sequences=False, input_shape = (X_TRAIN.shape[1], 6)))
...
model.save(f'../saved_model/BCH_model_for_minutes_from_full_data__val_lose_{val_loss[-1]}')
dump(sc, f'../saved_model/BCH_scaler_for_model_for_full_data_minutes_val_lose_{val_loss[-1]}')

save_info(ONE_BATCH_SIZE,BATCH_SIZE,EPOCHS,TRAIN_TEST_SPLIT_POINT,loss,val_loss,model,'BCH','full_data_minutes')
