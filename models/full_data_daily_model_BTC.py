...
# Load Dataframe
data = load_standard_data_frame(get_all_historical_data('BTC'))
...
# Build LSTM model
model = Sequential()
model.add(LSTM(units=16, return_sequences=False, input_shape = (X_TRAIN.shape[1], 6)))
...
model.save(f'../saved_model/BTC_model_for_days_from_full_data__val_lose_{val_loss[-1]}')
dump(sc, f'../saved_model/BTC_scaler_for_model_for_full_data_days_val_lose_{val_loss[-1]}')

save_info(ONE_BATCH_SIZE,BATCH_SIZE,EPOCHS,TRAIN_TEST_SPLIT_POINT,loss,val_loss,model,'BTC','full_data_days')
