...
# def get_data_with_limit_and_type(data_type, limit=29, crypto_currency='BTC', real_currency='USD'):
#     if data_type == 'days':
#         response_frame = pd.DataFrame(get_all_historical_data_with_limit(limit), columns=['close'])
#     elif data_type == 'hours':
#         response_frame = pd.DataFrame(get_hourly_last_3_months_data_with_limit(limit), columns=['close'])
#     elif data_type == 'minutes':
#         response_frame = pd.DataFrame(get_minute_last_day_data_with_limit(limit, crypto_currency, real_currency), columns=['close'])
#     else:
#         raise NameError('Zly argument')
#
#     response_frame = response_frame.values.tolist()
